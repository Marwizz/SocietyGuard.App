...
import {
  ...
  cabExit,
  deliveryExit,
} from "../services/operations/exitApi";
...
function GuestTabScreen({ societyId, refreshing, onRefresh, isLoading }) {
  ...
  const fetchGuestVisitors = async () => {
    try {
      const response = await visitorExit(societyId);
      setVisitors(response?.data?.data || []);
      console.log("exit guest visitors are ", response?.data?.data)
    } catch (error) {
...
function CabTabScreen({ societyId, refreshing, onRefresh, isLoading }) {
  ...
  const fetchCabVisitors = async () => {
    try {
      const response = await cabExit(societyId);
      setCabVisitors(response?.data?.data || []);
      console.log("exit cab visitors are ", response?.data?.data)
    } catch (error) {
...
function DeliveryTabScreen({ societyId, refreshing, onRefresh, isLoading }) {
  ...
  const fetchDeliveryVisitors = async () => {
    try {
      const response = await deliveryExit(societyId);
      console.log("exit delivery visitors are ", response?.data?.data)
...
export default function ExitScreen({ route }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Exit Visitors</Text>
      </View>
